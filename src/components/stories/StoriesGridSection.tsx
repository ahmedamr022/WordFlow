...
export default function StoriesRightPanel() {
  return (
    <div className="flex flex-col gap-4 w-full">
      ...
      {/* 2. كارت متابعة القراءة */}
      <div className="bg-[#080c17] border border-white/[0.07] rounded-[20px] p-4 shadow-2xl">
        <div className="font-extrabold text-white text-base mb-3">
          متابعة القراءة
        </div>

        <div className="relative rounded-xl overflow-hidden bg-gradient-to-b from-slate-900/20 to-[#080c17]/95 border border-white/[0.08] p-4 min-h-[110px] flex flex-col justify-end mb-3 bg-cover bg-center" style={{ backgroundImage: `linear-gradient(180deg, rgba(15,23,42,0.3) 0%, rgba(8,12,23,0.95) 100%), url('https://images.unsplash.com/photo-1509281373149-e957c6296406?w=400')` }}>
          ...
          <div className="text-xs text-slate-300 mb-3 text-right">
            أسطورة السفينة التايتانيك
          </div>

          <div className="flex items-center gap-3">
            <div className="flex-1 h-1.5 bg-white/15 rounded-full overflow-hidden">
              <div className="h-full w-[65%] bg-gradient-to-r from-indigo-400 to-purple-400 rounded-full" />
            </div>
            <span className="text-xs font-extrabold text-white">65%</span>
          </div>
        </div>

        <button className="w-full bg-gradient-to-r from-purple-900 to-rose-900 hover:from-purple-800 hover:to-rose-800 border border-rose-500/30 text-white py-2.5 rounded-xl font-extrabold text-sm shadow-lg shadow-rose-950/40 transition-all duration-200 hover:-translate-y-0.5">
          متابعة القصة
        </button>
      </div>
