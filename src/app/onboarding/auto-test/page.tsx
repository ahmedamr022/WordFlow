...
import { ArrowLeft, Volume2, Award, Zap, BrainCircuit, Loader2 } from "lucide-react";
import { AudioService } from "@/lib/audio/kokoroTTS";
import { savePlacementTestAction } from "@/app/actions/auth";
import type { CefrLevel } from "@/types/database";

/**
 * قبل: نتيجة اختبار تحديد المستوى كانت تُكتب في
 * localStorage["wordflow_level"] + UserStatsService.setLevel() — أي أنها
 * **لم تصل Supabase أبداً**، رغم وجود جدول placement_tests و
 * savePlacementTestAction جاهزين. المستخدم كان يفقد مستواه بتغيير المتصفح.
 * بعد: savePlacementTestAction (zod + assertSameOrigin + rate limit) يكتب
 * صف الاختبار ويحدّث profiles.english_level في نفس العملية.
 */
...
export default function AutoPlacementTestPage() {
  ...
  const handleNext = () => {
    ...
    // نحفظ الإجابة نفسها أيضاً — placement_tests.answers يخزّنها للتحليل لاحقاً.
    const updatedAnswers: RecordedAnswer[] = [
    ...userAnswers,
  ...
  const getCalculatedLevel = (): CefrLevel => {
  ...
  const finishTest = async () => {
    ...
    const result = await savePlacementTestAction({
      answers: userAnswers.map((a) => ({ questionId: a.questionId, answer: a.answer })),
      score: userAnswers.filter((a) => a.isCorrect).length,
      resulting_level: getCalculatedLevel()
    });
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-[#07090e] text-white font-arabic dir-rtl selection:bg-sky-400">
        <div className="w-full max-w-lg p-8 rounded-3xl glass-card border border-sky-400/40 text-center animate-in zoom-in-95 duration-300 shadow-2xl space-y-6">
          ...
          {saveError &&
          <p role="alert" className="text-sm text-red-400 bg-red-500/10 border border-red-500/30 rounded-xl p-3">
              {saveError}
            </p>
          }
          ...
          <button
            ...
            disabled={isSaving}
            className="w-full flex items-center justify-center gap-2 p-4 rounded-full bg-gradient-to-r from-sky-500 to-cyan-400 hover:from-sky-400 hover:to-cyan-300 text-slate-950 font-extrabold transition-all shadow-xl shadow-sky-500/30 active:scale-95 text-base disabled:opacity-60 disabled:active:scale-100">
            
            {isSaving ?
            <>
                <Loader2 className="w-5 h-5 animate-spin" aria-hidden />
                <span>جارٍ حفظ نتيجتك...</span>
              </> :
      ...
      <div className="w-full max-w-xl p-8 rounded-3xl glass-card border border-slate-800 shadow-2xl animate-in fade-in duration-300 space-y-6">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            <div>
              <h1 className="font-extrabold text-base text-white">اختبار WordFlow التكيفي (10 أسئلة)</h1>
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-900 border border-slate-800 text-xs font-mono text-slate-300">
            <BrainCircuit className="w-4 h-4 text-sky-400" aria-hidden />
        ...
        {/* Options Grid */}
        <div className="space-y-3 dir-ltr" role="radiogroup" aria-label="خيارات الإجابة">
        ...
        <p className="sr-only" aria-live="polite">
          سؤال {currentIndex + 1} من {TEN_ADAPTIVE_QUESTIONS.length}
        </p>
      </div>
    </div>);
