...
export default function RegisterPage() {
  ...
  const handleRegister = (e: React.FormEvent) => {
    e.preventDefault();
    localStorage.setItem("wordflow_nickname", name);
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-[#07090e] text-white font-arabic dir-rtl selection:bg-[#ff6b6b]">
      <div className="w-full max-w-md p-8 rounded-3xl glass-card border border-slate-800 shadow-2xl animate-in fade-in zoom-in-95 duration-300 relative">
        <div className="flex flex-col items-center justify-center mb-6">
          <Logo size={36} />
          <h1 className="text-3xl font-extrabold text-white mt-3 font-sans">إنشاء حساب جديد</h1>
          <p className="text-xs text-slate-400 mt-1">انضم إلى WordFlow وابدأ رحلتك التفاعلية في التعلم</p>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full p-4 pl-12 rounded-2xl bg-slate-900 border border-slate-800 text-white font-arabic focus:outline-none focus:border-sky-400 transition-all"
              />
              <User className="w-5 h-5 text-slate-500 absolute right-4 top-1/2 -translate-y-1/2" />
            </div>
          </div>
